<?php
		...
		if($_GET['action'] == 'saveForPreview') {
			if(is_numeric($_POST['slideshow_id'])) {
				...
				if(cu3er__isFallback($baseUrl . '/fallback')) {
					$query = mysql_query("SELECT `image`, `position`, `id`, `use_image` FROM `". $wpdb->prefix ."cu3er__slides` WHERE `slideshow_id`='". $_POST['slideshow_id'] ."'");
					while($row = mysql_fetch_assoc($query)) {
						$oldSlide[$row['id']] = $row;
					}
	...
	function cu3er__checkDatabaseForUpdate() {
		...
		switch($r['version']) {
			...
			case '0.30':
				...
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__defaults` ADD COLUMN `use_post_images` varchar(5)  NOT NULL DEFAULT '' AFTER `sheight`, ADD COLUMN `seo_show_paragraph` varchar(5)  NOT NULL DEFAULT '' AFTER `seo_show_heading`, ADD COLUMN `seo_show_caption` varchar(5)  NOT NULL DEFAULT '' AFTER `seo_show_paragraph`");
			case '0.31':
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__settings` ADD COLUMN `licence1` varchar(255)  AFTER `licence`, ADD COLUMN `licence2` varchar(255)  AFTER `licence1`, ADD COLUMN `licence3` varchar(255)  AFTER `licence2`, ADD COLUMN `licence4` varchar(255)  AFTER `licence3`;");
			case '0.32':
				mysql_query("UPDATE `" . $wpdb->prefix . "cu3er__slideshows` SET `project_location`=''");
			case '0.33':
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__defaults` ADD COLUMN `use_post_images` varchar(5)  NOT NULL DEFAULT '' AFTER `sheight`, ADD COLUMN `posts_count` int(11) NOT NULL DEFAULT 4 AFTER `seo_show_image`");
			case '0.34':
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__defaults` ADD COLUMN `posts_fit` varchar(255)  NOT NULL DEFAULT 'default' AFTER `sheight`, ADD COLUMN `fit` varchar(255) NOT NULL DEFAULT '' AFTER `seo_show_image`");
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__slides` ADD COLUMN `fit` varchar(255)  NOT NULL DEFAULT '' AFTER `use_image`;");
				break;
			case '0.35':
				mysql_query("ALTER TABLE `" . $wpdb->prefix . "cu3er__slideshows` ADD COLUMN `force_js3d` enum('no', 'yes')  NOT NULL DEFAULT 'no' AFTER `project_location`,
 ADD COLUMN `responsive` enum('yes', 'no')  NOT NULL DEFAULT 'yes' AFTER `force_js3d`, ADD COLUMN `height_type` enum('px', '%') NOT NULL DEFAULT 'px' AFTER `height`, ADD COLUMN `width_type` enum('px', '%') NOT NULL DEFAULT 'px' AFTER `width`");
				mysql_query("INSERT INTO `" . $wpdb->prefix . "cu3er__settings` (`id`,`version`) VALUES (1,'0.36') ON DUPLICATE KEY UPDATE `version`='0.36'");
				break;
			case '0.36':
	...
	function display_cu3er($slideshow, $ret = false) {
		if(is_numeric($slideshow)) {
			...
			$query = mysql_query("SELECT `use_post_images`,`posts_count` FROM `".$wpdb->prefix."cu3er__defaults` WHERE `slideshow_id`='".$slideshow."'") or die(mysql_error());
			$defaults = mysql_fetch_assoc($query);
